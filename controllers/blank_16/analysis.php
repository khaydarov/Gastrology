...
if (isset($_POST['add']))
{
	...
	$sp = $_POST['sp'];
	$sc = $_POST['sc'];
	$tarih_bemori_kliniki = $_POST['tarih_bemori_kliniki'];
	$ntt = $_POST['ntt'];

	if (checkAnalisys($_POST['sid']) == 1)
		$sql = "UPDATE spart8 SET am='$am', av='$av', sp='$sp', sc='$sc', tarih_bemori_kliniki='$tarih_bemori_kliniki', ntt='$ntt' 
																WHERE id='$spart8'";
	else 
		$sql = "INSERT INTO spart8(am, av, sp, sc, tarih_bemori_kliniki)
						VALUES('$am', '$av', '$sp', '$sc','$tarih_bemori_kliniki')";

	query($sql);


	/*==================================== TAHLILI KLINIKII BEMORI ===============================*/
	$tkb = $_POST['tkb'];
	$data_tkb = date('d.m.y H:i:s');
	$tkb_sid = $_POST['sid'];
	$s_uid = $_SESSION['duhtur_id'];


	if ($tkb != '') 
	{
		$sql = "INSERT INTO cstory(sid, uid, comment, data)
					VALUES('$tkb_sid', '$s_uid', '$tkb', '$data_tkb')";
	...
	/*========== SPART 8 ===========*/
		$sql = "SELECT id FROM spart8 ORDER BY id DESC LIMIT 1";
		$res = query($sql);
		$row = mysql_fetch_object($res);
		...
		$sql = "INSERT INTO story (spart1, spart2, spart3, spart4, spart5, spart6, spart7, spart8, sid)
		                    VALUES('$id1', '$id2', '$id3', '$id4', '$id5', '$id6', '$id7', '$id8', '$sid')";
		query($sql);
